...
import { Check, Star, Shield, Truck, Sparkles, ArrowRight, ChevronRight, Calculator } from 'lucide-react';
...
export default function PriceCalculator() {
    ...
    const Card = ({ title, desc, price, badge, highlight, onClick, active, icon: Icon }) => (
        <button
            onClick={onClick}
            className={`relative p-6 rounded-2xl border transition-all duration-300 w-full text-left group overflow-hidden flex flex-col h-full hover:shadow-2xl hover:-translate-y-1
                ${active
                    ? 'border-red-500 bg-red-900/20 shadow-[0_0_30px_rgba(220,38,38,0.15)] ring-1 ring-red-500/50'
                    : highlight
                        ? 'border-yellow-600/30 bg-gradient-to-br from-zinc-900/80 to-yellow-900/10 hover:border-yellow-500/50 hover:bg-zinc-800'
        ...
        >
            ...
            <div className="flex items-start justify-between mb-4 relative z-10">
                ...
                {Icon && <Icon className={`w-6 h-6 transition-colors duration-300 ${active ? 'text-red-500' : 'text-zinc-600 group-hover:text-red-400'}`} />}
            </div>

            <div className="text-zinc-400 text-sm leading-relaxed mb-6 flex-grow relative z-10 font-medium">{desc}</div>
    ...
    const renderResult = () => {
        ...
        const WhatsAppButton = ({ message }) => (
            <a
                ...
                className="flex items-center justify-center gap-3 bg-[#25D366] hover:bg-[#20bd5a] text-white px-6 py-4 rounded-xl font-bold transition-all duration-300 shadow-[0_0_20px_rgba(37,211,102,0.2)] hover:shadow-[0_0_40px_rgba(37,211,102,0.4)] hover:-translate-y-1 w-full mb-6 group border border-white/10"
            >
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="shrink-0 group-hover:scale-110 transition-transform duration-300"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
                Beratung & Termin
            </a>
        );
    ...
    return (
        <div className="relative w-full max-w-5xl mx-auto my-12" ref={containerRef}>
            ...
            <div className="relative bg-zinc-900/60 backdrop-blur-2xl rounded-3xl shadow-2xl overflow-hidden border border-white/10 p-6 md:p-12 scroll-mt-32 ring-1 ring-black/5">

                {/* Header Section */}
                <div className="text-center mb-12">
                    <div className="inline-flex items-center justify-center p-3 bg-white/5 rounded-2xl mb-6 shadow-lg border border-white/5">
                        <Calculator className="w-8 h-8 text-red-500" />
                    </div>
                    <h2 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">Kostenrechner</h2>
                    <p className="text-zinc-400 max-w-xl mx-auto text-lg">Konfigurieren Sie Ihr Paket. Fair, transparent und ohne versteckte Kosten.</p>
                </div>
                ...
                <div className="min-h-[400px] transition-all duration-500 ease-in-out">
                    ...
                    {step === STEPS.CAMPER_LENGTH && (
                         <div className="animate-fade-in">
                            ...
                            <div className="max-w-xl mx-auto bg-black/40 p-10 rounded-3xl border border-white/10 shadow-xl backdrop-blur-md">
                                <div className="text-center mb-10">
                                ...
                                <div className="relative mb-12 px-2">
                                    <input
                                        ...
                                        className="w-full h-3 bg-zinc-800 rounded-full appearance-none cursor-pointer accent-red-600 hover:accent-red-500 transition-all"
                                ...
                                <button
                                    ...
                                    className="w-full bg-red-600 hover:bg-red-500 text-white font-bold py-5 rounded-2xl transition-all duration-300 shadow-[0_10px_30px_rgba(220,38,38,0.3)] hover:shadow-[0_15px_40px_rgba(220,38,38,0.4)] flex items-center justify-center gap-3 transform hover:-translate-y-1"
                                >
                    ...
                    {step === STEPS.RESULT && !submitted && quote && (
                        <div className="animate-fade-in">
                            ...
                            <form onSubmit={submitQuote} className="max-w-lg mx-auto bg-white/5 p-8 rounded-3xl border border-white/10 backdrop-blur-sm">
                                ...
                                <div className="flex gap-3 flex-col">
                                    <input
                                        ...
                                        className="w-full bg-zinc-950/50 border border-white/10 text-white rounded-xl px-5 py-4 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 disabled:opacity-50 transition-all placeholder:text-zinc-600"
                                        value={email}
                                    ...
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="w-full bg-zinc-100 hover:bg-white text-black px-6 py-4 rounded-xl font-bold transition-all duration-300 disabled:opacity-50 whitespace-nowrap border border-transparent hover:scale-[1.02] flex items-center justify-center gap-2 shadow-lg"
                                    >
                    ...
                    {step === STEPS.RESULT && submitted && (
                        <div className="animate-fade-in text-center py-20">
                            ...
                            <button onClick={reset} className="px-8 py-4 bg-white/5 hover:bg-white/10 text-white rounded-xl transition-all border border-white/10 font-bold uppercase tracking-wider text-sm hover:border-white/20">
                                Neue Berechnung
                            </button>
                        </div>
                    )}
