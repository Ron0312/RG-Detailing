...
export default function PriceCalculator() {
    ...
    const Card = ({ title, desc, price, badge, highlight, onClick, active, icon: Icon }) => (
        <button
            onClick={onClick}
            className={`relative p-8 md:p-6 rounded-2xl border transition-all duration-300 w-full text-left group overflow-hidden flex flex-col h-full hover:shadow-2xl hover:-translate-y-1 min-h-[160px]
                ${active
    ...
    const renderResult = () => {
        ...
        const WhatsAppButton = ({ message }) => (
            <a
                ...
                className="flex items-center justify-center gap-3 bg-[#25D366] hover:bg-[#20bd5a] text-white px-6 py-4 rounded-xl font-bold transition-all duration-300 shadow-[0_0_20px_rgba(37,211,102,0.2)] hover:shadow-[0_0_40px_rgba(37,211,102,0.4)] hover:-translate-y-1 w-full mb-6 group border border-white/10 text-lg"
            >
                            ...
                            <form onSubmit={submitQuote} className="max-w-lg mx-auto bg-white/5 p-6 md:p-8 rounded-3xl border border-white/10 backdrop-blur-sm">
                                ...
                                <div className="flex gap-3 flex-col">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="w-full bg-zinc-100 hover:bg-white text-black px-6 py-4 rounded-xl font-bold transition-all duration-300 disabled:opacity-50 whitespace-nowrap border border-transparent hover:scale-[1.02] flex items-center justify-center gap-2 shadow-lg text-lg"
                                    >
