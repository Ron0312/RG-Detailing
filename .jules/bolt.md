...
## 2026-03-05 - [Scroll Event Layout Thrashing Avoidance]
**Learning:** High-frequency scroll event listeners that continually read layout properties (`scrollHeight`, `clientHeight`) force the browser to recalculate layout on every scroll event, leading to layout thrashing.
**Action:** Cache these expensive layout properties in a `useRef` and only update them when necessary via `ResizeObserver` or window `resize` events, instead of calculating them within the scroll event loop.