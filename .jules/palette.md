...
## 2025-03-20 - Add title attribute to icon-only buttons
**Learning:** Icon-only interactive elements with `aria-label` are accessible to screen readers, but sighted users might not immediately understand their function without context. A native hover tooltip is a simple UX enhancement that is very useful.
**Action:** When creating or modifying icon-only buttons or links, ensure that a `title` attribute is present alongside the `aria-label` attribute to display a helpful tooltip on hover.
