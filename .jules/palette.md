...
## 2025-03-09 - Focus styles for Astro navigation components
**Learning:** The main navigation elements in Astro components (like `Header.astro`), including the site logo link, desktop links, and mobile menu toggles, lacked `focus-visible` utility classes. This makes keyboard navigation completely invisible, especially against dark backgrounds where default browser focus rings do not provide enough contrast.
**Action:** Always explicitly apply `focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none` along with appropriate `ring-offset` classes to all interactive navigation elements (`<a>` and `<button>`) to ensure consistent, highly-visible keyboard accessibility across the application.
