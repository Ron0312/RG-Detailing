...
## 2024-03-09 - Accessible Expandable Components
**Learning:** Found that expandable content toggles lacked explicit ID bindings via `aria-controls`, preventing screen reader users from understanding what content was being toggled. Also, generic buttons lacked `type='button'` and visible focus indicators for keyboard users against dark backgrounds.
**Action:** Always use `useId()` to generate unique IDs for content containers and associate them with the toggle button via `aria-controls`. Also explicitly set `type="button"` and apply `focus-visible` utility classes for keyboard navigation visibility.
